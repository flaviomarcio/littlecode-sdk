...
import com.littlecode.setup.SetupMetaIndexes;
...
public class MetaDataClasses {
    ...
    public static final String FORMAT_CREATE_INDEX = "create %s index %s on %s(%s)".toUpperCase();
    ...
    public static class MetaTable implements SetupClassesDB.ObjectBase {
        ...
        public void setSchemaName(String newValue) {
            this.schemaName = newValue == null ? "" : newValue.trim();
        }
        ...
        public List<MetaField> getFields() {
            ...
            for (Field field : modelFields) {
                var metaField = new MetaField(field);
        ...
        public List<SetupMetaIndex> getMetaIndexes() {
            List<SetupMetaIndex> __return = new ArrayList<>();
            for (var method : this.tableMetaType.getMethods()) {
                if (!method.isAnnotationPresent(SetupMetaIndexes.class))
                    continue;

                if (!Modifier.isStatic(method.getModifiers())) {
                    log.info("{}: method[{}] ignored, method it is not static", SetupMetaIndexes.class.getSimpleName(), method.getName());
                    continue;
                }
                ...
                try {
                    ...
                    var list = (List<SetupMetaIndex>) method.invoke(null);
                    if (list != null && !list.isEmpty()) {
                        for (var index : list) {
                            if (PrimitiveUtil.isEmpty(index.getFinalName(tableName)))
                                log.error("{}: method[{}] ignored, reason: getFinalName is empty, body: {}", SetupMetaIndexes.class.getSimpleName(), method.getName(), ObjectUtil.toString(index));
                            else if (index.getFields().isEmpty())
                                log.error("{}: method[{}] ignored, reason: fields is empty, body: {}", SetupMetaIndexes.class.getSimpleName(), method.getName(), ObjectUtil.toString(index));
                            else
                                __return.add(index);
                        }
                    }
                } catch (InvocationTargetException | IllegalAccessException e) {
                    log.error("{}: method[{}] {}", SetupMetaIndexes.class.getSimpleName(), method.getName(), e.getMessage());
                }
            }
            return __return;
        }
