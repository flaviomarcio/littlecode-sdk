...
public class DataSourceBase {
    ...
    public HibernateJpaVendorAdapter vendorAdapter() {
        ...
        vendorAdapter.setDatabase(this.getDataSourceEnv().getDbType());
    ...
    public static class DataSourceEnv {
        ...
        public DataSourceEnv(DataSourceBase dataSourceBase) {
            ...
            setting = new DataSourceSetting(dataSourceBase.environment, this.getDataSourceSetting());

            var dbType = setting.getDbType();
    ...
    public static class DataSourceSetting {
        ...
        public DataSourceSetting(Environment environment, String DS) {
            if (environment == null)
            ...
            if (this.driverClassName.isEmpty()) {//TODO CHECK TO REMOVE
                if (this.getDbType().equals(Database.ORACLE))
        ...
        public Database getDbType() {
            var url = this.getUrl();
            if (url == null || !url.trim().isEmpty() || !url.startsWith("jdbc:"))
                return Database.H2;

            var parts = List.of(url.split(":"));
            if (parts.size() <= 1)
                return Database.H2;

            var dbType = parts.get(1).toUpperCase();
            if (dbType.isEmpty())
                return Database.H2;

            try {
                return Database.valueOf(dbType);
            } catch (Exception e) {
                return Database.H2;
            }
        }
